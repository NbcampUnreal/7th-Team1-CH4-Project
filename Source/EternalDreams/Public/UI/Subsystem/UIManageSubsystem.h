...
UCLASS()
class ETERNALDREAMS_API UUIManageSubsystem : public ULocalPlayerSubsystem
{
	GENERATED_BODY()
...
public:
	...
	void SetHUDLayoutClass(TSubclassOf<UHUDLayout> InHUDLayoutClass);
	...
	UHUDLayout* GetHUDLayout() const;

protected:
	// 실제로 생성된 HUD 위젯 인스턴스를 보관
	UPROPERTY(Transient)
	TObjectPtr<UHUDLayout> HUDLayoutInstance;

	// 생성에 사용할 HUD 위젯 클래스를 보관
	UPROPERTY(Transient)
	TSubclassOf<UHUDLayout> HUDLayoutClass;
